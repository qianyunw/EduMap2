...
import com.example.admin.edumap.adapter.BusResultListAdapter;
import com.example.admin.edumap.constant.CurrentLocationSingleton;
import com.example.admin.edumap.layout.LocationLayout;
...
public class RouteActivity extends Activity implements OnMapClickListener,
	...
	protected void onCreate(Bundle bundle) {
		...
		final Serializable serializableMap = (Serializable) bundle1.get("serializableMap");
		Map schoolInfo = (Map)serializableMap;
		if(schoolInfo != null) {
			Gson gson = new Gson();
			Map schoolDistrictMiddlepointTemp = gson.fromJson(schoolInfo.get("sMiddlepoint").toString(), new TypeToken<Map>(){}.getType());
			mEndPoint = new LatLonPoint(Double.parseDouble(schoolDistrictMiddlepointTemp.get("lat").toString()), Double.parseDouble(schoolDistrictMiddlepointTemp.get("lng").toString()));
			mStartPoint = new LatLonPoint(CurrentLocationSingleton.getInstance().getCurrentLat(), CurrentLocationSingleton.getInstance().getCurrentLon());
			RelativeLayout routemap_header = (RelativeLayout) findViewById(R.id.routemap_header);
			routemap_header.setVisibility(View.VISIBLE);
			LocationLayout location = (LocationLayout) findViewById(R.id.location);
			location.setVisibility(View.GONE);
			LinearLayout second_bottom_bar = (LinearLayout) findViewById(R.id.second_bottom_bar);
			second_bottom_bar.setVisibility(View.GONE);
			LinearLayout second_bottom_sheet = (LinearLayout) findViewById(R.id.second_bottom_sheet);
			second_bottom_sheet.setVisibility(View.GONE);
	...
	private void setfromandtoMarker() {
		LatLng startPoint = AMapUtil.convertToLatLng(mStartPoint);
		LatLng endPoint = AMapUtil.convertToLatLng(mEndPoint);
		...
		//对于比例尺的计算可以不需复杂逻辑，每个区case下便可。
		aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng((startPoint.latitude + endPoint.latitude) / 2, (startPoint.longitude + endPoint.longitude) / 2), 12.7f));
